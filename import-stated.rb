...
Dir.mkdir(STATED_SAVED_DIR) unless File.exist?(STATED_SAVED_DIR)
...
sets.each do |set|
  ...
  File.open(setdir + '.json', 'w') do |file|
    ...
    name.each_with_index do |name, i|
      url = format(STATED_URL, set: set, name: name)
      url = Addressable::URI.encode(url)

      if i == 0
        file.write format(STATED_ENTRY_FIRST, name: name.chomp('.png'), scale: scale, url: url, tag: statedtag,
                                              rotation: rotation)
        ...
        file.write format(STATED_ENTRY, name: name.chomp('.png'), statenum: i + 1, url: url, scale: scale,
                                        rotation: rotation)
      end
    end
